...
internals.error = function(error) {
  if (error.isBoom) {
    return internals.buildResponse(error.output.statusCode, error.output.headers, error.output.payload, error.data);
  }
...
internals.buildResponse = function(statusCode, headers, body, data) {
  if (data) {
    const _body = {
      ...body,
      data
