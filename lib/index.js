...
internals.setupQuery = function(event) {
  const queryStringParameters = Hoek.clone(Hoek.reach(event, 'queryStringParameters', {default: {}}));
  const multiValueQueryStringParameters = Hoek.reach(event, 'multiValueQueryStringParameters', {default: {}});
  ...
  event.query = mergeWith(queryStringParameters, multiValueQueryStringParameters, mergeCustomizer);
};
